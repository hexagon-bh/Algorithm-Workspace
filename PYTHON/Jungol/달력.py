...
def spawn_sub():
    ...
    title=tkinter.Label(sub,text="Title: ",font=("Arial",10))
    title.grid(row=1,column=0)
    input_text =tkinter.Entry(sub, width=30)
    input_text.grid(column=1, row=1)
    button =tkinter.Button(sub, text="확인", command=confirm)
...
def f_month_1():
    '''
    calendar.monthrange(year,month): (start week, last date)
    월요일 = 0
    화요일 = 1
    수요일 = 2
    목요일 = 3
